...
{
    public abstract class AppointmentAppTestBase : AbpIntegratedTestBase<AppointmentAppTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AppointmentAppTestBase()
        {
        ...
        protected void UsingDbContext(Action<AppointmentAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AppointmentAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AppointmentAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AppointmentAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AppointmentAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AppointmentAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AppointmentAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AppointmentAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AppointmentAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AppointmentAppDbContext, Task<T>> func)
        {
            T result;
